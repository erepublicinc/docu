...
echo "<h1> Loads data from Clickability </h1>";
echo " types:  gtauthor govauthor article<br>";

if(empty($_GET['type']))
   die(" need:   ?type=   <br>");

$url = "http://www.govtech.com/templates/getdata?pw=z&type=".$_GET['type'];
...
try {
   ...
   // $result = $xml->xpath('channel/item'); 
    ...
    { 
       switch((string) $node->content_type) 
       {
           case 'article': newArticle($node); break;
           case 'page':    newPage($node); break;
           case 'author':  newAuthor($node); break;
           default:   echo " <br> ======================== UNKNOWN content type: (string) $node->content_type <br> ";
       }     
    }
}
catch (Exception $e)
{
    die("<br> ************ Exception ". (string) $e);
...
function newArticle($node)
{
    // dump($node,false);   dump( (string) $node->abody,false); return;  
    $data = array(
    'contents_clk_id'       => (string) $node->clk_id[0],
    'contents_title' 		=> (string) $node->title,
    'contents_display_title'=> (string) $node->display_title,
    'contents_authors_id'   => 1,
    'contents_rev_status'   => 'LIVE',
    'contents_rev_comment'  => 'Created by loader',
    'contents_pub_date'     => (string) $node->pub_date,
    'contents_summary'      => (string) $node->summary,
 	'contents_article_body' => (string) $node->abody,
 	'contents_article_type'	=> 'article'
    );
    
    if( containsGarbage($data['contents_clk_id'],  $data['contents_article_body']))
       return;
...
function newAuthor($node)
{
    // dump($node,false);   dump( (string) $node->abody,false); return;  
    echo("author: ". (string) $node->name." <br>");
    
    $data = array(
    'authors_clk_id'        => (string) $node->clk_id,
    'authors_name' 		    => (string) $node->name,
    'authors_display_name'  => (string) $node->display_name,
    'authors_users_id'      => 1,
    'authors_role_title'    => (string) $node->role_title,
    'authors_summary'       => (string) $node->summary,
 	'authors_bio'           => (string) $node->bio,
 	'authors_public_email'  => (string) $node->email,
    'authors_twitter_url'   => (string) $node->twitter_url,
    'authors_googleplus_url'=> (string) $node->google_url,
    'authors_active'        => 1
    );
    
    if( containsGarbage($data['authors_clk_id'],  $data['authors_bio']))
       return;
    
    $item = new Author($data);
    $id = $item->Save();
...
function newPage($node)
{
 return;  //not ready yet
    $data = array(
        'pages_title'        => 'page integration test'.$randomId,
        ...
        'pages_rev_comment'  => 'Created by loader'
    );
...
function containsGarbage($id, $txt)
{
    // this seems to be a good test for stuff copied from word
    if(stripos($txt, "<xml>") !== false)
    {
       echo "<br>************** item: $id contains garbage<br>\n"; 
       return true;
    }   
    return false;    
}
